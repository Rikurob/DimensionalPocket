...
public class DimensionalPocketStructureAdditionalGenerationConditionProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		...
		if ((world instanceof Level _lvl ? _lvl.dimension() : Level.OVERWORLD) == (ResourceKey.create(Registry.DIMENSION_REGISTRY,
			...
			if ((entity.getCapability(DimensionalpocketModVariables.PLAYER_VARIABLES_CAPABILITY, null)
					.orElse(new DimensionalpocketModVariables.PlayerVariables())).pocketSpawn) {
				if (world instanceof ServerLevel _serverworld) {
					StructureTemplate template = _serverworld.getStructureManager()
							.getOrCreate(new ResourceLocation("dimensionalpocket", "dimensionalpockettype1"));
					if (template != null) {
